...
Obie badane funkcje osiągają minimum [równe 0](https://en.wikipedia.org/wiki/Test_functions_for_optimization#Test_functions_for_single-objective_optimization).
...
analizy ich położenia i rozproszenia.
...
Najmniejszy poziom istotności, przy którym dana wartość prowadzi do odrzucenia hipotezy
$H_0$, zwany także p-wartością wynosi:

\begin{equation}
p(t) = 2 \cdot P(T \geq |t| | H_0) = 2 \cdot (1 - F_{n - 1}(|t|))
\end{equation}

gdzie $F_{n - 1}$ jest dystrybuantą $t_{n-1}$.
...
Algorytm PRS
```{r}
t.test(prs_rosenbrock_2D, mu = 0, conf.level = 0.95, alternative = "two.sided")
```

Algorytm GA
```{r}
t.test(ga_rosenbrock_2D, mu = 0, conf.level = 0.95, alternative = "two.sided")
```
...
```{r}
t.test(prs_rosenbrock_10D, mu = 0, conf.level = 0.95, alternative = "two.sided")
```
Algorytm GA

```{r}
t.test(ga_rosenbrock_10D, mu = 0, conf.level = 0.95, alternative = "two.sided")
```
...
```{r}
t.test(prs_rosenbrock_20D, mu = 0, conf.level = 0.95, alternative = "two.sided")
```
Algorytm GA

```{r}
t.test(ga_rosenbrock_20D, mu = 0, conf.level = 0.95, alternative = "two.sided")
```
...
```{r}
t.test(prs_rastrigin_2D, mu = 0, conf.level = 0.95, alternative = "two.sided")
```
Algorytm GA
```{r}
t.test(ga_rastrigin_2D, mu = 0, conf.level = 0.95, alternative = "two.sided")
```
...
Algorytm PRS
```{r}
t.test(prs_rastrigin_10D, mu = 0, conf.level = 0.95, alternative = "two.sided")
```
Algorytm GA

```{r}
t.test(ga_rastrigin_10D, mu = 0, conf.level = 0.95, alternative = "two.sided")
```
...
Algorytm PRS
```{r}
t.test(prs_rastrigin_20D, mu = 0, conf.level = 0.95, alternative = "two.sided")
```
Algorytm GA
```{r}
t.test(ga_rastrigin_20D, mu = 0, conf.level = 0.95, alternative = "two.sided")
```
